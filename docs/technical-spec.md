...
- для target-merged `DefinedType` итоговый `Properties/Type` должен сериализоваться как `xr:ExtendedProperty`: `xr:CheckValue` содержит базовый `TypeDescription` из `target.xml_dump`, `xr:ExtendValue` — только добавленные `Native`-типы расширения; flattened union внутри обычного `<Type>` недопустим
- при такой сериализации у самого `DefinedType` обязателен согласованный `InternalInfo`: нужен `xr:GeneratedType` с `TypeId` / `ValueId`, а `xr:PropertyState` для `Type` должен существовать ровно один раз и иметь `State=MultiState`; если существующие `TypeId` / `ValueId` уже есть в source или target, их нужно сохранять
