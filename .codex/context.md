...
- объекты из `included_Native_objects` должны участвовать в тех же helper-ветках, что и prefix-native, но для самой классификации их нужно хранить отдельно как explicit native override над soft-exclude
...
- для merge-объектов `DefinedType` / `ExchangePlan` / `EventSubscription` из `CommonTemplate.упо_MetaDataFile` используется отдельный режим `AdoptedStubExtMetaData`: top-level объект остается adopted, не минимизируется до пустого stub, сохраняет `Properties/Type`, `Ext/Content.xml` или `Properties/Source`, участвует в target-ref-driven ссылках, но не переносит формы и BSL
- если задан `target.xml_dump`, для `DefinedType` / `EventSubscription` / `ExchangePlan` сначала собирается `targetCompatibilitySet`: `Native`-объекты этих типов допустимы всегда, adopted-объекты допустимы только если их top-level XML реально существует в target; коллектор читает только `DefinedTypes/*.xml`, `EventSubscriptions/*.xml`, `ExchangePlans/*.xml`
...
- Top-level объект из `included_Native_objects` должен оставаться `Native` даже если он попал в soft-excluded набор; при этом `forbidden_AdoptedStub_objects` все равно сильнее такого explicit include
- Top-level объект, который связан с веткой из `excluded_subsystems` или явно перечислен в `excluded_objects`, должен попадать в `Excluded` раньше обычного `Native` по native-prefix
...
- `AdoptedStubExtMetaData` — отдельный термин для merge-объектов из `CommonTemplate.упо_MetaDataFile`: только `DefinedType`, `ExchangePlan`, `EventSubscription`; они не `Native`, не являются alias обычного `AdoptedStub`, сохраняют `Type` / `Content` / `Source` и являются источником target-ref-driven ссылок, но не должны сохранять формы и BSL
- `targetCompatibilitySet` — отдельный термин для compatibility filter по target-sensitive объектам `DefinedType`, `EventSubscription`, `ExchangePlan`; он ограничивает adopted-состав наличием top-level XML в target, но сам по себе не делает `target.xml_dump` глобальным source graph
...
- source-ссылки таких `DefinedType` / `ExchangePlan` / `EventSubscription` до merge не должны становиться обычным `RefDrivenInclusion`; до и после promotion для них дополнительно действует `targetCompatibilitySet`, а в самом merge эти объекты идут как `AdoptedStubExtMetaData`; target-ссылки из их сохраненного `Type` / `Content` / `Source` могут дотянуть отсутствующий top-level metadata-объект из `target.xml_dump` как обычный `AdoptedStub`, но не переигрывают `forbidden_*`, не возвращают adopted target-sensitive объект вне `targetCompatibilitySet` и не делают target глобальным source graph
- если adopted top-level metadata-object совпал с объектом в `target.xml_dump` по top-level `id/uuid`, но target использует другое имя, canonical именем для extension считается target-name; stale source-name нельзя оставлять вторым adopted-объектом с тем же `InternalInfo`
